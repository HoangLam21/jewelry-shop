...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const getAllProducts = async () => {
      try {
        ...
        if (isMounted) {
          // Filter products that have this voucher
          const filtered = data.filter((product: ProductResponse) =>
            product.vouchers?.some((v: VoucherInProduct) => v._id === id)
          );
  ...
  const handleSortChange = (option: SortOption) => {
    ...
    const sortedData: ProductResponse[] = [...productsData].sort(
        ...
        {displayedItems.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-xl text-gray-500">
              No products found with this voucher
            </p>
            <Link href="/products">
